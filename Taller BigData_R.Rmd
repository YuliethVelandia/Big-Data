...
```{r}
df_viviendas_lz <- df_viviendas_lz %>% mutate(tipo_vivienda = ifelse(UA_CLASE %in% c("1","2"),"Urbana","Rural"))
```
...
consulta_etnia <- grupos_etnias %>% select(COD_ENCUESTAS, UA_CLASE, PA1_GRP_ETNIC, Nombre_Grupo) %>% right_join(df_viviendas_lz) %>% filter(PA1_GRP_ETNIC==1 | PA1_GRP_ETNIC==5)%>% select(-COD_ENCUESTAS) %>%  group_by(tipo_vivienda,Nombre_Grupo) %>% summarise(cuenta = n())
```  
